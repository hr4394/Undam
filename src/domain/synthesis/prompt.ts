...
export function buildSystemPrompt(): string {
  return [
    ...
    "",
    "[분량과 밀도]",
    "- 짧게 끝내지 말고, 각 항목을 충분히 길고 풍성하게 쓰세요. 핵심 통찰(coreInsights)은 3~4개, 각 항목은 여러 문장으로 구체적으로 채우세요.",
    "- 단, 같은 말을 반복해 분량만 부풀리지는 마세요. 새로운 구체적 관찰로 채우세요.",
    "",
    "[해석 원칙]",
...
export function buildUserPrompt(input: SynthesisInput): string {
  ...
  return [
    ...
    "일반인이 술술 읽을 수 있도록 길고 풍성하게(대략 2,500~4,000자) 쓰되, 어려운 용어에는 괄호로 쉬운 설명을 붙이세요.",
    "핵심 통찰(coreInsights)은 3개 이상 담고, 각 통찰의 근거(사주/별자리)를 반드시 함께 제시하세요.",
  ].join("\n");
}
