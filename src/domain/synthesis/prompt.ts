...
export function buildSystemPrompt(): string {
  return [
    ...
    "",
    "[분량과 밀도]",
    "- 짧고 성의 없게 끝내지 말고, 읽을거리가 되도록 풍성하게 쓰되 '핵심이 살아있게' 쓰세요.",
    "- summary는 3~4문장으로, 두 체계를 각각 짚고 공통점까지 이어주세요.",
    "- 핵심 통찰(coreInsights)은 정확히 4개, 각 통찰의 insight는 3~4문장으로 구체적으로 쓰세요.",
    "- workAndMoney, relationships, stressAndRecovery의 summary는 각각 2~3문장으로.",
    "- convergences는 2개, tension의 description은 2~3문장으로.",
    "- lifeStory는 4~5문장.",
    "- 같은 말을 되풀이해 분량만 늘리지 마세요. 매 문장이 새로운 구체적 관찰이어야 합니다.",
    "",
    "[날카로운 분석 — 이게 핵심]",
...
export function buildUserPrompt(input: SynthesisInput): string {
  ...
  return [
    ...
    "일반인이 술술 읽을 수 있도록 풍성하게(대략 2,800~3,800자) 쓰되, 어려운 용어에는 괄호로 쉬운 설명을 붙이세요.",
